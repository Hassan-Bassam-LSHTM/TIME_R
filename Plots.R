...
n_state <- n_dis*81*(1+n_HIV+n_HIV*n_ART)

# Demography #####################################################################################################

## Load and manipulate UN numbers 
## Load UN population data
UN_pop_age <- as.data.frame(read.table(paste("Demog/",cntry,"/",cntry,"_pop_age.txt",sep=""),header=TRUE)) # Load UN Population data
...
model_temp[,18] <- tot[,81]

# Then aggregate into 5 year bins used in TIME
for (i in 1:16){
  t1 <- (i)+(i-1)*4
  t2 <- t1+4
  model_temp[,i+1] <- rowSums(tot[,t1:t2])
}
...
plot_pop <- ggplot(dat_to_plot,aes(x=Year,y=value,colour=Model))+
  ...
  ggtitle(paste(cntry,"Population (by 5 year age bins), total births and total deaths",sep=" - "))+
  xlim(c(1970,2050))+
  ylab("Thousands")+theme_bw()

# TB outputs #####################################################################################################

# Mortality (HIV- and HIV+ lines) - need to calculate mortality by HIV status in model
# Prevalence (All)
...
WHO_out <- as.data.frame(read.table(paste("TB/",cntry,"/",cntry,"_WHO_TB.txt",sep=""),header=TRUE,fill=TRUE))

# Arrange model output
R_out <- as.data.frame(cbind(out[,"time"],
                             100000*out[,"TB_deaths_neg"]/out[,"Total"], # Mort (HIV-)
                             100000*out[,"TB_deaths_pos"]/out[,"Total"], # Mort (HIV+)
...
colnames(R_out) <- c("Year","V1","V2","V3","V4","V5","V6")
R_out <- R_out[R_out$Year<=2050,]

# combine and melt
Models_out <- R_out
Models_out <- melt(Models_out,id=c("Year"))
Models_out <- cbind(Models_out,c(rep("Mortality",2*81),rep("Prevalence",81),rep("Incidence",3*81)),c(rep("HIV-",81),rep("HIV+",81),rep("All",2*81),rep("HIV+",81),rep("Notifications",81)))
...
plot_TB <- ggplot(Models_out)+
  ...
  xlim(c(1990,2020))+
  ylab("Rate /100,000")+
  ggtitle(paste(cntry,"TB burden",sep=" - "))+
